...
class LabWiki::Gimi < OMF::Base::LObject
  ...
  def init_user(user)
    ...
    when "OpenID.GENI"
      ...
      if (urn = user['http://geni.net/user/urn'].first)
        OMF::Web::SessionStore[:urn, :user] = urn.gsub '|', '+'
        OMF::Web::SessionStore[:id, :user] = urn && urn.split('|').last
      end
      if (irods_user = user['http://geni.net/irods/username'].first)
        OMF::Web::SessionStore[:id, :irods_user] = irods_user
      end
      if (irods_zone = user['http://geni.net/irods/zone'].first)
        OMF::Web::SessionStore[:id, :irods_zone] = irods_zone
      end
    when "OpenID.Google"
      last_name = user["http://axschema.org/namePerson/last"].first
      first_name = user["http://axschema.org/namePerson/first"].first
      pretty_name = "#{first_name} #{last_name}"
      OMF::Web::SessionStore[:id, :user] = user["http://axschema.org/contact/email"].first
    end
