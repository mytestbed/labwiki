  ...
  LW.plugin.experiment.experiment_monitor = function(exp_name) {
    ...
    OHUB.bind('data_source.log_' + exp_name + '.changed', function(ev) {
      ...
      _.each(msgs.slice(0).reverse(), function(e) {
        ...
        if (message.match(/^Experiment .+ finished after .+$/)) $("#btn-stop-experiment").hide();
    ...
    $("#btn-dump").click(function(event) {
      ...
      $.post("/dump", { domain: $(this).attr("exp_id")}, function(data) {
        $(".alert-dump").html(data);
        $(".alert-dump").addClass("alert-success");
        $(".alert-dump").removeClass("alert-error");
      }).fail(function(data) {
        $(".alert-dump").html(data.responseText);
        $(".alert-dump").addClass("alert-error");
        $(".alert-dump").removeClass("alert-success");
