...
define(['theme/labwiki/js/labwiki', 'plugin/plan_text/js/scrollspy2'], function (lw, scrollspy) {
  ...
  var plan_text_monitor = function(content, wid) {
    ...
    b.share = pc.add_toolbar_button({name: 'share', awsome: 'share-square', tooltip: 'Share Page', active: true},
      function(ctxt) {
        var opts = {
          action: 'share',
          col: 'plan',
    ...
    text_el.find('.embedded .caption').each(function() {
      var c = $(this);
      var dc = c.parent().find('.omf_data_widget_container');
      var wid = dc[0].id;

      OHUB.bind('column.plan.resize', function(e) {
        var widget = OML.widgets[wid];
        widget.resize();
      });

      var cloner = OML.widget_proto[wid];
      var caption = c.find('span.text').text();
      var clones = 0;
      c.append('<span class="fullscreen_icon glyphicon glyphicon-fullscreen" />');
      c.find('.fullscreen_icon').click(function() {
        var cid = "wc" + wid + (clones += 1);
        LW.show_modal(caption, null, function(body) {
          body.append($("<div id='" + cid + "' class='omf_data_widget_container' />"));
          cloner(cid);
        });
        return false;
      });
