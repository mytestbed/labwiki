...
define(['theme/labwiki/js/labwiki', 'plugin/plan_text/js/scrollspy2'], function (lw, scrollspy) {
  ...
  var plan_text_monitor = function(content, wid) {
    ...
    b.share = pc.add_toolbar_button({name: 'share', awsome: 'share-square', tooltip: 'Share Page', active: true},
      function(ctxt) {
        ctxt.configure({awsome: 'spinner fa-spin', label: 'Submitting...'});
        var opts = {
          action: 'share',
          col: 'plan',
          plugin: 'wiki',
        ...
        LW.plan_controller.request_action(opts, 'POST', function(reply) {
          ctxt.configure({awsome: 'share-square'});
          if (reply.success != undefined) {
            controller.show_alert('success', opts.url + ' submitted');
          } else {
            if (reply.error != undefined) { controller.show_alert('alert', reply.error); }
          }
          // ???
          var i = 0;
        });
